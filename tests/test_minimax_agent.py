...
def test_minimax_agent_depth_1_is_greedy():
    """At depth 1 minimax picks the highest immediate score move."""
    game = _make_game()
...
def test_minimax_agent_avoids_floor():
    """At depth 1, agent prefers moves that don't add floor tiles when
    a pattern line move is available with equal or better immediate score."""
    game = Game()
    game.setup_round()
    agent = MinimaxAgent(depth=1)
    move = agent.choose_move(game)
    ...
    if non_floor:
        # Should not go to floor if a non-floor move scores at least as well
