...
def _make_game_for_bug_report() -> Game:
    ...
    _place_wall(p1._wall, 0, T.BLUE)
    _place_wall(p1._wall, 0, T.YELLOW)
    _place_wall(p1._wall, 0, T.RED)
    _place_wall(p1._wall, 0, T.BLACK)
    _place_wall(p1._wall, 0, T.WHITE)

    _place_wall(p1._wall, 1, T.BLUE)
    _place_wall(p1._wall, 1, T.YELLOW)
    _place_wall(p1._wall, 1, T.RED)
    _place_wall(p1._wall, 1, T.BLACK)

    _place_wall(p1._wall, 2, T.BLUE)
    _place_wall(p1._wall, 3, T.WHITE)
    _place_wall(p1._wall, 4, T.RED)
    _place_wall(p1._wall, 4, T.BLACK)

    p1._floor_line.extend([Tile.FIRST_PLAYER, Tile.YELLOW, Tile.YELLOW])
    p1._update_pending()
    p1._update_penalty()
    p1._update_bonus()
    ...
    _place_wall(p2._wall, 0, T.BLUE)
    _place_wall(p2._wall, 0, T.YELLOW)
    _place_wall(p2._wall, 0, T.BLACK)
    _place_wall(p2._wall, 0, T.WHITE)

    _place_wall(p2._wall, 1, T.WHITE)
    _place_wall(p2._wall, 1, T.BLUE)
    _place_wall(p2._wall, 1, T.YELLOW)
    _place_wall(p2._wall, 1, T.RED)
    _place_wall(p2._wall, 1, T.BLACK)

    _place_wall(p2._wall, 2, T.BLACK)
    _place_wall(p2._wall, 2, T.WHITE)
    _place_wall(p2._wall, 2, T.RED)

    _place_wall(p2._wall, 3, T.YELLOW)

    _place_wall(p2._wall, 4, T.WHITE)
    _place_wall(p2._wall, 4, T.BLUE)

    p2._floor_line.clear()
    p2._update_pending()
    p2._update_penalty()
    p2._update_bonus()
